...
namespace bBall
{
	...
	public partial class PlayBTPage : ContentPage, IMenuContainerPage
    {
        ...
        async void OnHeaderButtonClicked(object sender, EventArgs e)
        {
            ...
            if (_bt_play.isConected)
            {
                await Navigation.PushAsync(new PlayDetailsPage(lData, _bt_play, _Device_name));
            }
        }
        ...
        async void Play()
        {
            ...
            _bt_play.conected_status = _bt_play.device.WhenStatusChanged().Subscribe((obj) =>
            {
                if (_bt_play.device.Status == ConnectionStatus.Connected)
                {
                    ...
                    //_bt_play.conected_dev = _bt_play.device.WhenReadRssiContinuously(TimeSpan.FromMilliseconds(150)).Subscribe((newrssi) =>
                    _bt_play.conected_dev = _bt_play.device.WhenRssiUpdated(TimeSpan.FromMilliseconds(150)).Subscribe((newrssi) =>
                    {
